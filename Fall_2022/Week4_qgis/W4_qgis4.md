...
<p align='center'>
<img src="../Images/kaggle_RE1.png" width="600">
</p>
...
Webscrapping can be used for all kinds of malicious purposes, for instance, to copy website content and republish it, or to get better market data or price data. Here's a [complaint from Craiglist](https://www.scribd.com/doc/313832868/CraigslistVRadpad-Complaint?secret_password=7gTybamKvrbeVhxfi4mx) about a company called Radpad scrapping Craigslist and reposting those listing on their own website:
...
**What about webscrapping for research or academic purposes?** Aside from these issues, which you mostly won't be subject to, webscrapping makes a website's traffic *spike* if you don't modulate how often you're pinging the website. This can cause the website's server to crash. This is not very nice. Also, a lot of websites won't allow you to do it. (If you go to any URL and put `/robots.txt` after it, you can see a list of websites will see what that site will or won't allow you to scrape.)

## 1.2 APIs and Geocoding
Geocoding is the process of transforming some spatial description of a place into geographical coordinates. We have already done the simpler kind of geocoding, which is taking table that already has geographic coordinates already in the form of lat/long columns and turning those into points. That's fairly straightforward.

Today, we're going to geocode **addresses** and circumvent standard geocoding methods. If you've taken an Intro to GIS course, you may learned that you need some kind of address locator and reference street dataset that needs to be properly styled and formatted so that your geocoder can properly interpolate between street numbers and find the location of an address.

We are going to learn a different method that takes advantage of Open Street Map's geocoding API, called [Nominatim](https://wiki.openstreetmap.org/wiki/Nominatim). Some things to note:
- There's a limitation of one request per second
- If there are typos in the addresses it might not get picked up
...
We are going to use the `n=10` sample to start. The original 923k sample is [here](https://www.dropbox.com/s/6uxgjney859894w/realtor-data.csv?dl=0), FYI.
...
- `city` has the city name. Sometimes NYC is `New York`, sometimes `New York City`. Perhaps `Brooklyn` is encoded separately? Hard to say.
- `state` is the state, thankfully there seems to be a consistent formatting, at least for this sample.
- `zip_code` is formatted as a number, which we already knew.
- There are some values in the other columns that are missing. Not ideal, though not the end of the world (for now).
- The last entry has no street address, so we're not going to expect that to be geocoded.
...
(I've hidden my google maps API key, which is not needed for Nominatim anyways.)
...
If there are duplicate responses, the default is to `Use Only First Result`, which we'll keep. If we think that there are some systematic ways that first results are wrong, we can switch the second `Multiple Features for Multiple Results` option.
...
- Opening up the attribute table of my `real-estate-ny-10sample_geocoded.shp`, I can see that the entries with addresses but not apartment names worked. Surprisingly, the one without a street address, only the city, also worked!
...
Now, geocode this **revised** `real-estate-ny-10sample_failed.csv` again. Make sure to change the output names from the original so you don't write over those 4 points. And name the second failed table as something else.

Woohoo! It looks like all 6 new entries worked.
...
<p align='center'>
<img src="../Images/bk_atlas.png" width="600">
<br>
A small section of northern Brooklyn
</p>

First, what are we even looking at. Here's an index with a legend. (Link [here](https://digitalcollections.nypl.org/items/6c05633e-3bdd-b9cf-e040-e00a18063bc7))
<p align='center'>
<img src="../Images/bk_index.png" width="600">
</p>
...
**The best way to georeference if you already know the projection of the image map.** We then set our projection to that (using the globe-hat). Since, I could not find the projection listed in this atlas, I'm just going to set my map to my default of `EPSG:3857`.

We are going to use one of our **QuickMapServices** basemaps to do the georeferencing. Even though these basemaps show the NYC of today, most of the significant reference points, like roads, parks, and buildings are mostly the same today as it was in 1916. Here I'm using the **OSM->OSM Standard** basemap. It doesn't look as pretty as some of the other ones we've been using, and that's because it clearly shows most of the physical features that will be useful for us.
...
3. Click on its location in the OSM map
...
Now do this for more (maybe 7-8?) points and make sure you have points evenly distributed across all the corners and edges. (Note: We do have some roads, such as the Brooklyn-Queens Expressway, and public housing developments that were built after 1916.)
...
#### DELIVERABLE #2
Finish georeferencing this image and create a PDF output of the georeferenced map with 40% transparency so we can see the basemap underneath.
...
#### DELIVERABLE #3
Create a proportional symbol map, output as a PDF, of the available price to square foot for your geocoded data. Make sure to include a scale bar, title, and map with a basemap. Also, as text on your map, answer the following:
